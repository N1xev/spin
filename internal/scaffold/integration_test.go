...
func assertAGENTSmd(t *testing.T, projectDir string) {
	...
	// Line 1 must NOT be an AUTOGENERATED marker — per CONTEXT D-12 the
	// owned-by-spin signal is retired; generated files look like user files.
	firstLine, _, _ := strings.Cut(string(agents), "\n")
	if strings.HasPrefix(firstLine, "<!-- AUTOGENERATED by spin ") {
		t.Errorf("AGENTS.md line 1 = %q, must not start with AUTOGENERATED marker (D-12)", firstLine)
	}
