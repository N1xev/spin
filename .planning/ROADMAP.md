...
**Plans**: 6 plans in 4 waves
  - [ ] **04-01-PLAN.md** (Wave 1): `spin doctor` core — internal/doctor/ package with 4 universal checks, --format human|json, --strict, --deep, --fix; cmd/doctor.go
  - [ ] **04-02-PLAN.md** (Wave 1): `spin lint` wrapper — internal/wrap/lint.go with golangci-lint detect + install hint; cmd/lint.go with ArbitraryArgs
  - [ ] **04-03-PLAN.md** (Wave 1): `spin update` engine — internal/update/{parse,resolve,apply}.go (modfile parse, proxy.golang.org fetch, go get + tidy + CGO=0 build; no go test)
  - [ ] **04-04-PLAN.md** (Wave 2): `spin update` huh v2 form + cmd — internal/update/form.go (multi-Select per dep, non-TTY table fallback); cmd/update.go with --all flag
  - [ ] **04-05-PLAN.md** (Wave 1): Strip `// generated by spin` and `<!-- AUTOGENERATED by spin` markers from 25 .tmpl files; invert TestAGENTSmd_MarkerOnLine1
  - [ ] **04-06-PLAN.md** (Wave 3): CI dogfood — .github/workflows/{ci,dogfood}.yml + scripts/dogfood.sh (local-runnable)
...
| 4. Post-Scaffold Health + Verification + Dogfooding | 0/6 | Not started | - |
