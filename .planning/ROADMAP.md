...
**Plans**: 4 plans in 2 waves
  - [ ] **05-01-PLAN.md** (Wave 1): Rust ecosystem - internal/ecosystems/rust/ with Flags, Validate, Render, PostScaffold, Tasks; registered in defaultRegistry; cmd/new_rust.go dispatch
  - [ ] **05-02-PLAN.md** (Wave 1): Ecosystem dispatch + template loader - cmd/new.go deprecation shim + ecosystem-name detection; upgraded internal/template/loader.go (XDG cache, GIT_TERMINAL_PROMPT=0); new post_hook.go with RunPostHook; RenderToWithPost deletes spin.toml; registry unit tests
